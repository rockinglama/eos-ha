...
decisions:
  ...
  - title: Single integration instance via unique_id
    rationale: EOS HA manages one optimization setup per HA instance; multiple instances would conflict
...
Created the foundational directory structure and Config Flow for the EOS HA Home Assistant custom integration. Users can now install via HACS and configure the integration through HA's UI with a 3-step wizard that validates the EOS server connection, selects energy data entities, and sets battery parameters.

### Task 1: HACS Integration Skeleton and Constants
**Commit:** 4c14d63

Created the `custom_components/eos_ha/` directory with HACS-compatible metadata and shared constants:

- **manifest.json**: Integration manifest with `config_flow: true`, `domain: eos_ha`, and `iot_class: local_polling`. No external requirements (aiohttp is built into HA).
...
[ -f "custom_components/eos_ha/manifest.json" ] && echo "FOUND: manifest.json" || echo "MISSING: manifest.json"
[ -f "custom_components/eos_ha/hacs.json" ] && echo "FOUND: hacs.json" || echo "MISSING: hacs.json"
[ -f "custom_components/eos_ha/const.py" ] && echo "FOUND: const.py" || echo "MISSING: const.py"
[ -f "custom_components/eos_ha/config_flow.py" ] && echo "FOUND: config_flow.py" || echo "MISSING: config_flow.py"
[ -f "custom_components/eos_ha/strings.json" ] && echo "FOUND: strings.json" || echo "MISSING: strings.json"
[ -f "custom_components/eos_ha/translations/en.json" ] && echo "FOUND: translations/en.json" || echo "MISSING: translations/en.json"

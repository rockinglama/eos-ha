...
must_haves:
  truths:
    - "User can see EOS Connect in the Add Integration UI after placing files in custom_components"
  ...
  artifacts:
    - path: "custom_components/eos_connect/manifest.json"
      provides: "Integration metadata for HA discovery"
      contains: "\"domain\": \"eos_connect\""
    - path: "custom_components/eos_connect/hacs.json"
      provides: "HACS compatibility metadata"
      contains: "\"name\": \"EOS Connect\""
    - path: "custom_components/eos_connect/const.py"
      provides: "Shared constants for all modules"
      contains: "DOMAIN"
    - path: "custom_components/eos_connect/config_flow.py"
      provides: "3-step Config Flow with EOS validation"
      contains: "async_step_user"
    - path: "custom_components/eos_connect/strings.json"
      provides: "UI strings for Config Flow"
      contains: "\"step\""
    - path: "custom_components/eos_connect/translations/en.json"
      provides: "English translations matching strings.json"
      contains: "\"step\""
  key_links:
    - from: "custom_components/eos_connect/config_flow.py"
      to: "custom_components/eos_connect/const.py"
...
<task type="auto">
  ...
  <action>
Create the directory structure `custom_components/eos_connect/` with these files:
...
- `"config_flow": true`
- `"dependencies": []`
- `"documentation": "https://github.com/ohAnd/EOS_connect"`
...
- Import DOMAIN from const
  ...
  <verify>
Run: `python -c "import json; d=json.load(open('custom_components/eos_connect/manifest.json')); assert d['domain']=='eos_connect'; assert d['config_flow']==True; print('manifest OK')"` and
`python -c "import ast; ast.parse(open('custom_components/eos_connect/const.py').read()); print('const.py OK')"`
  </verify>
  <done>
...
- On submission: merge all data and create config entry with `self.async_create_entry(title="EOS Connect", data=self.data)`
...
- `config.step.user.title`: "EOS Connect"
...
- `config.abort.already_configured`: "EOS Connect is already configured."
- `config.abort.no_home_location`: "Home location not configured in Home Assistant. Set your home location in Settings > System > General."
  ...
  <verify>
Run: `python -c "import ast; tree=ast.parse(open('custom_components/eos_connect/config_flow.py').read()); classes=[n.name for n in ast.walk(tree) if isinstance(n, ast.ClassDef)]; assert 'EOSConnectConfigFlow' in classes; print('config_flow.py OK')"` and
`python -c "import json; s=json.load(open('custom_components/eos_connect/strings.json')); assert 'config' in s; assert 'step' in s['config']; assert 'user' in s['config']['step']; assert 'entities' in s['config']['step']; assert 'battery' in s['config']['step']; print('strings.json OK')"` and
`python -c "import json; t=json.load(open('custom_components/eos_connect/translations/en.json')); assert 'config' in t; print('translations OK')"`
  </verify>
  <done>
...
<verification>
1. Directory structure exists: `custom_components/eos_connect/` with all 7 files
2. `manifest.json` is valid JSON with `config_flow: true` and `domain: eos_connect`
3. `hacs.json` is valid JSON with `name: EOS Connect`

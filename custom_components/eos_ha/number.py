...
EV_NUMBERS: tuple[EOSNumberEntityDescription, ...] = (
    EOSNumberEntityDescription(
        key="ev_capacity",
        translation_key="ev_capacity",
    ...
    EOSNumberEntityDescription(
        ...
        native_step=100,
        mode=NumberMode.BOX,
        config_key=CONF_EV_CHARGE_POWER,
    ),
)
...
_DEFAULTS = {
    ...
    CONF_EV_CAPACITY: DEFAULT_EV_CAPACITY,
    CONF_EV_CHARGE_POWER: DEFAULT_EV_CHARGE_POWER,
}
...
async def async_setup_entry(
    ...
    ev_enabled = entry.options.get(CONF_EV_ENABLED, entry.data.get(CONF_EV_ENABLED, False))
    if ev_enabled:
        entities.extend(EOSNumber(coordinator, entry, desc) for desc in EV_NUMBERS)
